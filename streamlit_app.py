...
# =============================================================================
# 4. 지점 (Store) 페이지 기능 - 월별 근무기록 관리 (시간 입력 유연성 개선)
# =============================================================================

# 새로운 헬퍼 함수: HHMM -> HH:MM 자동 변환 및 최종 유효성 검사
def _format_time_input(time_input):
    """사용자의 시간 입력(HHMM 또는 HH:MM)을 표준 HH:MM 형식으로 변환하고 유효성을 검사합니다."""
    s = str(time_input).strip()
    
    # HHMM 형식 (4자리 숫자)인 경우 -> HH:MM 으로 변환
...
def render_store_attendance(user_info):
    ...
    with col2:
        ...
        selected_date = st.date_input("관리할 날짜 선택 (상세 관리)", 
                                      value=default_date, min_value=start_date, max_value=end_date,
                                      key="date_selector")
    st.markdown("---")
    ...
    def calculate_duration(row):
        try:
            # _format_time_input을 거치므로 HH:MM 형식이라고 확신할 수 있음
    ...
    if month_records_df.empty:
            ...
            for _, emp in store_employees_df.iterrows():
                ...
                for dt in pd.date_range(start_date, end_date):
                    if dt.weekday() in work_day_indices:
                        record_id = f"manual_{dt.strftime('%y%m%d')}_{emp['이름']}_{int(datetime.now().timestamp())}_{len(new_records)}"
        ...
        required_cols = ['정상근무', '연장근무']; [summary.update({col: 0}) for col in required_cols if col not in summary]
        ...
        if st.button(f"💾 {selected_date.strftime('%m월 %d일')} 기록 저장", type="primary", use_container_width=True):
            ...
            if not error_found:
                # (시간 겹침 검사 및 저장 로직은 이전과 동일하나, 'processed_df'를 사용)
            ...
            if not error_found:
                ...
                new_details = processed_df.copy() # 변환된 데이터프레임을 사용
